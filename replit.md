...
## Recent Changes
- **November 6, 2025**: Enhanced Hero section vacancy animation with multi-step countdown (9 → 8 → 6 vagas) and improved interactivity. The number now animates through three stages: starts at 9, transitions to 8 after 3 seconds, then to 6 after 6 seconds. When reaching 6, a red pulsing effect loops infinitely to create urgency. Added an accessible, translucent tooltip that appears on hover (desktop) or click/tap (mobile) displaying "Atualizado em tempo real" with a pulsing green indicator dot. The pill is now keyboard-accessible (Enter/Space keys) with proper ARIA labels and focus styles for full accessibility compliance.
